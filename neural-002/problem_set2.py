...
def bin_spikes(trials, spk_times, time_bin):
    ...
    for t in trials[:, 1]:
        ub = t + time_bin
        lb = t - time_bin
        tmp.append(sum((spk_times < ub)&(spk_times >= lb)))

    spike_cts = np.array(tmp)
    ...
    for n in directions:
        ...
        dir_trials = (trials[:,0] >= n)&(trials[:,0] < (n+45))
        dir_ct = sum(dir_trials)
        dir_time = dir_ct*time_bin*2
        dir_spikes = sum(spike_cts[dir_trials])
        tmp.append(dir_spikes/dir_time)

    dir_rates = np.column_stack((directions, np.array(tmp)))
...
def roll_axes(direction_rates):
    ...
    Hint: Use np.roll()
    """
    m, n = direction_rates.shape
    direction_rates = np.row_stack((direction_rates, direction_rates[0, :]))
    direction_rates[8,0] = 360

    steps_to_shift =  4 - direction_rates[:,1].argmax()
    roll_degrees = steps_to_shift*45

    new_ys =  np.roll(direction_rates[:, 1], steps_to_shift)

    new_xs = np.array([-1*roll_degrees + n*45 for n in range(9)])
